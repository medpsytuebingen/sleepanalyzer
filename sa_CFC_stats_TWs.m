...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
% performs statistics for theta wave CFC 
%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

function [results] = sa_CFC_stats_TWs(gcfg, GA_TW_freq, GA_TW_timelock, All_TW_freq, All_TW_timelock, TW_tsv)  
...
if opt.tfrstats  
    ...
    All_zero_freq = All_TW_freq;
    for i = 1:length(All_TW_freq) % create fake zero data
        All_zero_freq{i}.powspctrm(~isnan(All_zero_freq{i}.powspctrm)) = 0;
    end    

    cfg = [];
    cfg.channel = 'all';
    cfg.latency = gcfg.plot_TW_xlim; % [begin end] in seconds; 'all'
    cfg.trials = 'all';
    cfg.frequency = gcfg.plot_TW_ylim; % [begin end]; 'all'
    cfg.parameter = 'powspctrm';   
    cfg.method = 'montecarlo';
    cfg.statistic = 'depsamplesT'; % dependent samples T-statistic	
    cfg.design(1,:) = [repmat(1:numel(All_TW_freq),1,2)]; % subject vector        
    cfg.design(2,:) = [ones(1,length(All_TW_freq)) ones(1,length(All_TW_freq))*2]; % condition vector        
    ...
    [stat] = ft_freqstatistics(cfg, All_TW_freq{:}, All_zero_freq{:}); 
    %     [stat] = ft_freqstatistics(cfg, All_TW_freq{:}, All_TW_surrogate_freq{:}); 
    %     [stat] = ft_freqstatistics(cfg, All_TW_freq{:}, All_TW_free_freq{:}); 
    ...
    if opt.clusterstats
        ...
        [stat_cor] = ft_freqstatistics(cfg, All_TW_freq{:}, All_zero_freq{:});     
        %     [stat_cor] = ft_freqstatistics(cfg, All_TW_freq{:}, All_TW_surrogate_freq{:}); 
        %     [stat_cor] = ft_freqstatistics(cfg, All_TW_freq{:}, All_TW_free_freq{:}); 
    end
    ...
    if opt.plot
        figure;
        subplot(4,1,1);
        plotdata = squeeze(GA_TW_freq.powspctrm(1,GA_TW_freq.freq >= cfg.frequency(1) & GA_TW_freq.freq <= cfg.frequency(2), GA_TW_freq.time >= cfg.latency(1) & GA_TW_freq.time <= cfg.latency(2)));
    ...
    %     title('thresholded t-values (p<0.05 cluster-corrected)'); 
        ...
        % save plot
        if opt.saveplot
            set(gcf, 'Name',[gcfg.conditionLabels{gcfg.thisCondition} '_GA_TW_' gcfg.TWspec '_' gcfg.TW_timelockevent '_' gcfg.TW_tsv_flag '_' TW_timelockchannel '_stats_' gcfg.timestamp],'NumberTitle','off','units','normalized','outerposition',[0 0 1 1]);
            filename = fullfile(gcfg.paths.results, 'group', [gcfg.conditionLabels{gcfg.thisCondition} '_GA_TW_' gcfg.TWspec '_' gcfg.TW_timelockevent '_' gcfg.TW_tsv_flag '_' TW_timelockchannel '_stats_' gcfg.timestamp]);
            saveas(gcf,[filename '.fig']);        
            set(gcf, 'PaperUnits', 'centimeters', 'PaperSize', [25.4 19.05], 'PaperPosition', [-3 -1 30 19.5]);
            print(gcf, '-dpdf', '-r300', [filename '.pdf']);   
        end
    CFC_results_TW.stat = stat;    
    end
...
if opt.pacpt % phase amplitude coupling per trial
    ...
    % settings
%     hpfilter = 0.25; lpfilter = 1.25; timewin = [-1.0 1.0]; freqwin = [12 16]; % for SW-spindle PAC 
    hpfilter = 2; lpfilter = 4; timewin = [-0.3 0.3]; freqwin = [80 100]; % for SW-gamma PAC 
%     
    gcfg.timewin = sprintf('[%2.2f %2.2fs]',timewin(1),timewin(2));
    gcfg.freqwin = sprintf('[%2.2f %2.2fHz]',freqwin(1),freqwin(2));
    ...
    for i = 1:numel(gcfg.subjects) % loop over sujects
        ...
        load(fullfile(gcfg.paths.results,subjectNames{i},[subjectNames{i} '_TW_' gcfg.TWspec '_' gcfg.TW_timelockevent '_' gcfg.TW_targetchannel{gcfg.subjects(i)} '_eventdata.mat']), 'TW_eventdata'); % load freq data
        load(fullfile(gcfg.paths.results,subjectNames{i},[subjectNames{i} '_TW_' gcfg.TWspec '_' gcfg.TW_timelockevent '_' gcfg.TW_targetchannel{gcfg.subjects(i)} '_freq.mat']), 'TW_freq'); % load freq data

        % select trials        
        cfg = [];
        cfg.trials = find(TW_tsv{i});
        cfg.tolerance   = 1e-3;
        TW_eventdata = ft_selectdata(cfg,TW_eventdata); 
        TW_freq = ft_selectdata(cfg,TW_freq);         
        
        % define data 
        freqindex = TW_freq.freq >= freqwin(1) & TW_freq.freq <= freqwin(2);     
        timeindex = unique(int16(find(TW_eventdata.time{1} >= timewin(1) & TW_eventdata.time{1} <= timewin(2))-1));
    
        trialN{i} = size(TW_eventdata.trial,2); % # trials        
        for j = 1:trialN{i} % loop over trials
            
            % define data for trial j
            LF = TW_eventdata.trial{j};
            HF = squeeze(mean(TW_freq.powspctrm(j,1,freqindex,:),3))';
            
            % preprocess LF data
            LFhp = ft_preproc_bandpassfilter(LF, double(TW_eventdata.fsample), [hpfilter lpfilter], 3*fix(TW_eventdata.fsample/hpfilter)+1, 'fir', 'twopass'); % Mathilde's formula
            
            % preprocess HF data
%             usfactor = double(TW_eventdata.fsample / round(1/mean(diff(TW_freq.time)))); %  upsampling factor to interpolate HF (TFR) signal to LF signal
            HF(isnan(HF)) = 0; % replace NaN by 0
%             HFus = interp(HF,usfactor); % upsample to fit LF time resolution
%             HFus = transpose(resample(transpose(HF),TW_eventdata.fsample,round(1/mean(diff(SO_freq.time))))); % upsample to fit LF time resolution
            HFus = transpose(resample(transpose(double(HF)), double(TW_freq.time), double(TW_eventdata.fsample), 'pchip')); % upsample to fit LF time resolution
            HFhp = ft_preproc_highpassfilter(HFus, double(TW_eventdata.fsample), hpfilter, 3*fix(TW_eventdata.fsample/hpfilter)+1, 'fir', 'twopass'); % Mathilde's formula        
            
            % LF event-locked HF power change (difference)
            bl = [-2.5 -1.5]; % for spindles it would be bl = [-2.0 -1.0]
            blindex = unique(int16(find(TW_eventdata.time{1} >= bl(1) & TW_eventdata.time{1} <= bl(2))));
...
%         [KSh{i},KSp{i},KSk{i},KSc{i}] = kstest([SIp{i}]',[[SIp{i}]', unifcdf([SIp{i}]',-pi,pi)]);
        ...
        % save figure
        set(gcf, 'Name',[subjectNames{i} '_TW_' gcfg.TWspec '_' gcfg.TW_timelockevent '_' gcfg.TW_tsv_flag '_' gcfg.TW_timelockchannel '_CFCtrialwise_' gcfg.timewin '_' gcfg.freqwin '_'  gcfg.timestamp],'NumberTitle','off','units','normalized','outerposition',[0 0 1 1]);
        filename = fullfile(gcfg.paths.results, subjectNames{i}, [subjectNames{i} '_TW_' gcfg.TWspec '_' gcfg.TW_timelockevent '_' gcfg.TW_tsv_flag '_' gcfg.TW_timelockchannel '_CFCtrialwise_' gcfg.timewin '_' gcfg.freqwin '_' gcfg.timestamp]);
        saveas(gcf,[filename '.fig']);   
        set(gcf, 'PaperUnits', 'centimeters', 'PaperSize', [25.4 19.05], 'PaperPosition', [-3 -1 30 19.5]);
        print(gcf, '-dpdf', '-r300', [filename '.pdf']);   
    ...
    if ~isempty(ft_channelselection({'eeg'},{gcfg.TW_timelockchannel})) % if EEG channel
        Vtestvalue = 0;
    elseif strcmp(gcfg.TW_timelockchannel,'HC')
        Vtestvalue = 180;
    end
    ...
    filename = fullfile(gcfg.paths.results, 'group', [gcfg.conditionLabels{gcfg.thisCondition} '_GA_TW_' gcfg.TWspec '_' gcfg.TW_timelockevent '_' gcfg.TW_tsv_flag '_' gcfg.TW_timelockchannel '_CFCtrialwise_' gcfg.timewin '_' gcfg.freqwin '_' gcfg.timestamp '.txt']);
    ...
    % save figure
    set(gcf, 'Name',[gcfg.conditionLabels{gcfg.thisCondition} '_GA_TW_' gcfg.TWspec '_' gcfg.TW_timelockevent '_' gcfg.TW_tsv_flag '_' gcfg.TW_timelockchannel '_CFCtrialwise_' gcfg.timewin '_' gcfg.freqwin '_' gcfg.timestamp],'NumberTitle','off','units','normalized','outerposition',[0 0 1 1]);
    filename = fullfile(gcfg.paths.results, 'group', [gcfg.conditionLabels{gcfg.thisCondition} '_GA_TW_' gcfg.TWspec '_' gcfg.TW_timelockevent '_' gcfg.TW_tsv_flag '_' gcfg.TW_timelockchannel '_CFCtrialwise_' gcfg.timewin '_' gcfg.freqwin '_' gcfg.timestamp]);
    saveas(gcf,[filename '.fig']);   
    set(gcf, 'PaperUnits', 'centimeters', 'PaperSize', [25.4 19.05], 'PaperPosition', [-3 -1 30 19.5]);
    print(gcf, '-dpdf', '-r300', [filename '.pdf']); 
    ...
    % save results
    filename = fullfile(gcfg.paths.results, 'group', [gcfg.conditionLabels{gcfg.thisCondition} '_GA_TW_' gcfg.TWspec '_' gcfg.TW_timelockevent '_' gcfg.TW_tsv_flag '_' gcfg.TW_timelockchannel '_CFCtrialwise_' gcfg.timewin '_' gcfg.freqwin '_' gcfg.timestamp '.mat']);
