...
% last modified 2017/07/25 by TOB
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
% detects slow oscillations in EEG data
%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

function [gcfg, data, bpdata] = sa_detectSOs(gcfg,data)
display('Detecting slow oscillations...');
...
%         if ~strcmp('Cz',bpdata.label{j}) % if NOT Cz and thus iEEC channel            
...
for i = 1:size(bpdata.trial,2) % loop over trials
    for j = find(ismember(bpdata.label, gcfg.searchchannel))' % loop over search channels
        for k = 1:numel(pos2neg0cross{i,j})-1 
            [SOtrough{i,j}(k) SOtroughTime{i,j}(k)]= min(bpdata.trial{i}(j,pos2neg0cross{i,j}(k):neg2pos0cross{i,j}(k)) .* pol(j));
            [SOpeak{i,j}(k) SOpeakTime{i,j}(k)] = max(bpdata.trial{i}(j,neg2pos0cross{i,j}(k):pos2neg0cross{i,j}(k+1)) .* pol(j));
            SOp2p{i,j}(k)= SOpeak{i,j}(k) - SOtrough{i,j}(k);
            SOdur{i,j}(k) = (pos2neg0cross{i,j}(k+1)-pos2neg0cross{i,j}(k))/bpdata.fsample;
            SOp2pTime{i,j}(k) = ((neg2pos0cross{i,j}(k)-pos2neg0cross{i,j}(k))-SOtroughTime{i,j}(k))+(SOpeakTime{i,j}(k))/bpdata.fsample;
        end
    end
end


%% determine amplitude threshold (only for candidate SOs of correct duration!!!)
...
switch gcfg.ampcritparameter
    ...
    case 'both'
        SOampcrit = SOtrough;
        SOampcrit2 = SOpeak; % will be used from here onwards as additional criterion whenever gcfg.ampcritparameter =='both' ist tested
end


% gather data for variance threshold calculation (only for candidate SOs of correct duration!!!)
display('Determining amplitude criteria from data.');
for i = 1:size(bpdata.trial,2) % loop over trials
    for j = find(ismember(bpdata.label, gcfg.gaugechannel))' % loop over gauge channels
        ec = 0;
        for k = 1:numel(SOampcrit{i,j}) % loop over SO candidates
            if all(finalGaugeFilter{i}(j,pos2neg0cross{i,j}(k):pos2neg0cross{i,j}(k+1))) % if SO candidate is located in artifact free data
                if SOdur{i,j}(k) >= gcfg.durcriterion(1) && SOdur{i,j}(k) <= gcfg.durcriterion(2) % duration criterion fullfilled
                    ec = ec+1;
                    vardata{j}(ec) = abs(SOampcrit{i,j}(k)); % use absolute (!) values of amplitude criterion variable
                    if strcmp(gcfg.ampcritparameter, 'both')
                        vardata2{j}(ec) = abs(SOampcrit2{i,j}(k)); % use absolute (!) values of 2nd amplitude criterion variable
                    end
                end
            end
        end
        switch gcfg.centerType
            case  'median'
                medianSOampcrit(j) = median(vardata{j},2);
                gcfg.center(j) = medianSOampcrit(j);
                if strcmp(gcfg.ampcritparameter, 'both')
                    medianSOampcrit2(j) = median(vardata2{j},2);
                    gcfg.center2(j) = medianSOampcrit2(j);
                end
            case  'mean'
                meanSOampcrit(j) = mean(vardata{j},2);
                gcfg.center(j) = meanSOampcrit(j);
                if strcmp(gcfg.ampcritparameter, 'both')
                    meanSOampcrit2(j) = mean(vardata2{j},2);
                    gcfg.center2(j) = meanSOampcrit2(j);
                end
            case  'none'
                minSOampcrit(j) = min(vardata{j},[],2);
                gcfg.center(j) = minSOampcrit(j);
                if strcmp(gcfg.ampcritparameter, 'both')
                    minSOampcrit2(j) = min(vardata2{j},2);
                    gcfg.center2(j) = minSOampcrit2(j);
                end
        end % of switch
        switch gcfg.varianceType
            case 'mad'
                madSOampcrit(j) = mad(vardata{j},1,2);
                gcfg.variance(j) = madSOampcrit(j);
                gcfg.SOampthresh(j) = gcfg.center(j) + gcfg.variance(j) .* gcfg.ampcriterion;
                if strcmp(gcfg.ampcritparameter, 'both')
                    madSOampcrit2(j) = mad(vardata2{j},1,2);
                    gcfg.variance2(j) = madSOampcrit2(j);
                    gcfg.SOampthresh2(j) = gcfg.center2(j) + gcfg.variance2(j) .* gcfg.ampcriterion;
                end
            case 'sd'
                sdSOampcrit(j) = std(vardata{j},1,2);
                gcfg.variance(j) = sdSOampcrit(j);
                gcfg.SOampthresh(j) = gcfg.center(j) + gcfg.variance(j) .* gcfg.ampcriterion;
                if strcmp(gcfg.ampcritparameter, 'both')
                    sdSOampcrit2(j) = std(vardata2{j},1,2);
                    gcfg.variance2(j) = sdSOampcrit2(j);
                    gcfg.SOampthresh2(j) = gcfg.center2(j) + gcfg.variance2(j) .* gcfg.ampcriterion;
                end
            case 'percent'
                ...
                gcfg.SOampthresh(j) = percentilevardata(j);
                if strcmp(gcfg.ampcritparameter, 'both')
...
if isfield(gcfg,'loadampcritvalues') && gcfg.loadampcritvalues == 1
    ...
    SOamthresh_loaded = []; SOamthresh2_loaded = []; center_loaded = [];
    for iFile = 1:length(gcfg.ampcritvalfiles)
        temp{iFile} = load(gcfg.ampcritvalfiles{iFile});
        SOamthresh_loaded(iFile,:) = temp{iFile}.SO_gcfg.SOampthresh;
        center_loaded(iFile,:) = temp{iFile}.SO_gcfg.center;
        if strcmp(gcfg.ampcritparameter,'both')
            SOamthresh2_loaded(iFile,:) = temp{iFile}.SO_gcfg.SOampthresh2;
        end
    end
    gcfg.SOampthresh = mean(SOamthresh_loaded,1);
    gcfg.center = mean(center_loaded,1);    
    if strcmp(gcfg.ampcritparameter,'both')
        gcfg.SOampthresh2 = mean(SOamthresh2_loaded,1);
    end
...
for i = 1:size(bpdata.trial,2) % loop over trials
    for j = find(ismember(bpdata.label, gcfg.searchchannel))' % loop over search channels
        ...
        for k = 1:numel(SOampcrit{i,j}) % loop over potential SOs
            if all(finalSearchFilter{i}(j, pos2neg0cross{i,j}(k):pos2neg0cross{i,j}(k+1))) % correct sleep stage and no artifact accordig to finalSearchFilter                
                if abs(SOampcrit{i,j}(k)) >= gcfg.SOampthresh(j); % amplitude criterion fullfilled
                    if strcmp(gcfg.ampcritparameter, 'both') % 2nd amplitude criterion required 
                        ok = abs(SOampcrit2{i,j}(k)) >= gcfg.SOampthresh2(j); % 2nd amplitude criterion fullfilled 
                    else
                        ok = 1; % if 2nd amplitude criterion NOT required
                    end
                    if ok % 2nd apmlitude criterion either fullfilled or not required
                        if SOdur{i,j}(k) >= gcfg.durcriterion(1) && SOdur{i,j}(k) <= gcfg.durcriterion(2) % duration criterion fullfilledif SOdur{i,j}(k) >= gcfg.durcriterion(1) && SOdur{i,j}(k) <= gcfg.durcriterion(2) % duration criterion fullfilled
                            ec = ec + 1;
                            ...
                            gcfg.eventInfo(i,j).duration(ec) = single(SOdur{i,j}(k));  % event duration (in seconds)
                            gcfg.eventInfo(i,j).maxTime(ec) = single(SOpeakTime{i,j}(k) + neg2pos0cross{i,j}(k)); % time of event peak (in datapoints)
                            gcfg.eventInfo(i,j).minTime(ec) = single(SOtroughTime{i,j}(k) + pos2neg0cross{i,j}(k)); % time of event trough (in datapoints)               
                            gcfg.eventInfo(i,j).minAmp(ec) = SOtrough{i,j}(k); 
                            gcfg.eventInfo(i,j).maxAmp(ec) = SOpeak{i,j}(k); 
                            gcfg.eventInfo(i,j).p2pAmp(ec) = SOp2p{i,j}(k); 
                            gcfg.eventInfo(i,j).p2pTime(ec) = single(SOp2pTime{i,j}(k)); % in seconds
    ...
    for i = 1:size(bpdata.trial,2) % loop over trials
        for j = find(ismember(rmsdata.label, gcfg.searchchannel))' % loop over search channels
            ...
            while runthis
                ...
                display(['Threshold ' num2str(thresh) ' µV: ' num2str(currnumepochs) ' of ' num2str(targetnumepochs) ' event-free epochs found.']);
...
for j = find(ismember(bpdata.label, gcfg.gaugechannel))' % loop over gauge channels        
    ...
    gcfg.summary.MEAN_SOp2p = mean(gcfg.eventInfo(1,j).p2pAmp);
    gcfg.summary.MEDIAN_SOp2p = median(gcfg.eventInfo(1,j).p2pAmp);
    gcfg.summary.MIN_SOp2p = min(gcfg.eventInfo(1,j).p2pAmp);
    gcfg.summary.MAX_SOp2p = max(gcfg.eventInfo(1,j).p2pAmp);
    
    gcfg.summary.SOampthresh(j) = gcfg.SOampthresh(j);
    
    numelvardata(j) = numel(vardata{j});
    sortvardata{j} = sort(vardata{j},2);
    gcfg.summary.Perc_MEAN_SOp2p(j) = find(sortvardata{j} >= gcfg.summary.MEAN_SOp2p,1,'first')/numelvardata(j);
    gcfg.summary.Perc_MEDIAN_SOp2p(j) = find(sortvardata{j} >= gcfg.summary.MEDIAN_SOp2p,1,'first')/numelvardata(j);
    gcfg.summary.Perc_MIN_SOp2p(j) = find(sortvardata{j} >= gcfg.summary.MIN_SOp2p,1,'first')/numelvardata(j);
    gcfg.summary.Perc_MAX_SOp2p(j) = find(sortvardata{j} >= gcfg.summary.MAX_SOp2p,1,'first')/numelvardata(j);
    gcfg.summary.Perc_SOampthresh(j) = find(sortvardata{j} >= gcfg.summary.SOampthresh(j),1,'first')/numelvardata(j);
...
for j = 1:numel(bpdata.label) 
    bpdata.label{j} = [bpdata.label{j} '_SO_bp']; % change channel names  
end
