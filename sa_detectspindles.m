...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
% detects sleep spindles in EEG data
%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

function [gcfg, data, bpdata, rmsdata, supthreshdata] = sa_detectspindles(gcfg,data)
display('Detecting spindles...');
...
%% determine spindle peak frequency
% % FFT
% cfg = [];
% cfg.method = 'mtmfft';
% cfg.output = 'pow';
...
% end
% 
% % select sleep spindle peak automatically
...
% % [gcfg.spindlePeakValue,index] = max(log(freq.powspctrm(:,searchIndex)+1).*repmat(freq.freq(searchIndex),size(freq.powspctrm,1),1).^2,[],2); % correct for 1/f distribution of EEG power spectrum to unambigiously detect spindle peak
% % [gcfg.spindlePeakValue,index] = max(log(smoothfreq(:,searchIndex)+1).*repmat(freq.freq(searchIndex),size(smoothfreq,1),1).^3,[],2); % correct for 1/f distribution of EEG power (^3!!) spectrum to unambigiously detect spindle peak
% % [gcfg.spindlePeakValue,index] = max(log(smoothfreq(:,searchIndex)+1).*repmat(freq.freq(searchIndex),size(smoothfreq,1),1),[],2); % correct for 1/f distribution of EEG power spectrum to unambigiously detect spindle peak
% [gcfg.spindlePeakValue,index] = max(log(smoothdifffreq(:,searchIndex)+1));
% gcfg.spindlePeakFreq = searchInterval(index);
% % save('-v7.3', fullfile(gcfg.resultspath,[gcfg.subjectName '_spindle_FFT_stage' char(gcfg.gaugestages+'0') '_' gcfg.timestamp]),'wakefreq', 'NREMfreq', 'difffreq'); 
...
%     xlabel({'Hz'});
%     ylabel('log(power+1)');
%     title([data.label{j} ', spindle freq: ' num2str(gcfg.spindlePeakFreq)]);
% end
% saveas(h, fullfile(gcfg.resultspath,[gcfg.subjectName '_spindle_FFT_stage' char(gcfg.gaugestages+'0') '_' gcfg.timestamp '.fig'])); 
% close(h);


%% bandpass filter in spindle range
bpdata = data;
if gcfg.searchfreqindividual == 1 % use individual spindle fequency filter
    gcfg.bpfreq = [gcfg.spindlePeakFreq' - repmat(gcfg.searchfreqmargin,length(gcfg.spindlePeakFreq),1) gcfg.spindlePeakFreq' +  repmat(gcfg.searchfreqmargin,length(gcfg.spindlePeakFreq),1)];
    for i = 1:size(data.trial,2) % loop over trials
...
elseif gcfg.searchfreqindividual == 0 % use common spindle fequency filter
...
if isfield(gcfg,'loadampcritvalues') && gcfg.loadampcritvalues == 1
    ...
    for iFile = 1:length(gcfg.ampcritvalfiles)
        ...
        rmsamthresh_loaded(iFile,:) = temp{iFile}.spindle_gcfg.rmsampthresh;
        center_loaded(iFile,:) = temp{iFile}.spindle_gcfg.center;
...
for i = 1:size(rmsdata.trial,2) % loop over trials
    for j = find(ismember(rmsdata.label, gcfg.searchchannel))' % loop over search channels
        ...
        ec = 0; % event counter
        for k = 1:length(startIndex) % loop over potential spindles
            if duration(k) >= gcfg.durcriterion(1)*data.fsample && duration(k) <= gcfg.durcriterion(2)*data.fsample % duration criterion fullfilled
                ...
                gcfg.eventInfo(i,j).maxTime(ec) = single(startIndex(k) + maxIndex); % time of spindle peak (in datapoints)
                gcfg.eventInfo(i,j).minTime(ec) = single(startIndex(k) + minIndex); % time of spindle trough (in datapoints)               
                ...
                supthreshdata{i}(j,startIndex(k):endIndex(k)) = 0; % remove false spindles from suprathreshdata
            end                 
        end
    end
end
        ...
        for j = find(ismember(rmsdata.label, gcfg.searchchannel))' % loop over search channels
            ...
            for k = 1:length(startIndex) % loop over potential spindles
                if duration(k) >= gcfg.durcriterion(1)*data.fsample && duration(k) <= gcfg.durcriterion(2)*data.fsample % duration criterion fullfilled
                    ...
                    [minAmp,minIndex] = min(searchWindow); 
                    [maxAmp,maxIndex] = max(searchWindow); 
                    gcfg.eventfreeInfo(i,j).maxTime(efc) = single(startIndex(k) + maxIndex); % time of spindle peak (in datapoints)
                    gcfg.eventfreeInfo(i,j).minTime(efc) = single(startIndex(k) + minIndex); % time of spindle trough (in datapoints)               
                    ...
                    gcfg.eventfreeInfo(i,j).RMSmaxTime(ec) = single(startIndex(k) + RMSmaxIndex); % time of spindle peak (in datapoints)
                else % duration criterion NOT fullfilled
                    supthreshdata_ef{i}(j,startIndex(k):endIndex(k)) = 0; % remove false spindles from suprathreshdata
                end                 
            end
        end
    end
...
for j = 1:numel(bpdata.label) 
    bpdata.label{j} = [bpdata.label{j} '_spindle_bp']; % change channel names  
end
for j = 1:numel(rmsdata.label) 
    rmsdata.label{j} = [rmsdata.label{j} '_spindle_rms']; % change channel names  
end
